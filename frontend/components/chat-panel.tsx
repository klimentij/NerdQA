...
export function ChatPanel({
  ...
  useEffect(() => {
    const fetchExampleMessages = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/example-messages')
        if (!response.ok) {
          throw new Error('Failed to fetch example messages')
        }
        const data = await response.json()
        setExampleMessages(data.example_messages)
      } catch (error) {
